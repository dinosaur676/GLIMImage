...
{
    if (width <= 0 || height <= 0) {
        LOGGER.LogError("ImageBuffer/invalid_argument", "ImageBuffer: width/height must be positive (got " +
    ...
    if (pixelCount > MAX_PIXEL_COUNT) {
        ...
        LOGGER.LogError("ImageBuffer/invalid_argument", "ImageBuffer: image too large (max " +
...
std::uint8_t* ImageBuffer::rowPtr(int y) {
    if (y < 0 || y >= m_height) {
        LOGGER.LogError("ImageBuffer/out_of_range", "ImageBuffer::rowPtr: y=" + std::to_string(y) +
