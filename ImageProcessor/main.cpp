...
int main(int argc, char* argv[]) {
    try {
        ...
        // ── CLI 인자 파싱 (제공된 코드) ─────────────────────────
        ip::ProgramOptions options = ip::CommandLineParser::parse(argc, argv);



        // ── BMP 로드 (제공된 코드) ──────────────────────────────
        ip::ImageBuffer image = ip::BmpParser::loadFromFile(options.options[ip::OptionIndex::INPUT].getArg());
        ...
        std::string filterCLI = options.optionString;

        Filters::getInstance().addFilter(filterCLI);
        ip::ImageBuffer resultImage = Filters::getInstance().runFilter(image);

        // ── BMP 저장 (제공된 코드) ──────────────────────────────
        ip::BmpParser::saveToFile(options.options[ip::OptionIndex::OUTPUT].getArg(), resultImage);
        std::cout << "Saved:  " << options.options[ip::OptionIndex::OUTPUT].getArg() << "\n";
        return 0;
    }
